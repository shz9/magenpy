...
- Documentation for the `magenpy` CLI scripts.
- Complete testing of the CLI scripts in `test_cli.sh`.

### Changed

- Removed `output_dir` from `GWADataLoader` initialization.

### Fixed

- Fixed CLI script documentation formatting issues.
- Fixed bugs in handling of backends in `mgp_compute_ld` and `mgp_simulate`.
