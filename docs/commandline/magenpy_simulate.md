...
usage: magenpy_simulate [-h] --bfile BED_FILE [--keep KEEP_FILE] [--extract EXTRACT_FILE] [--backend {magenpy,plink,bed-reader,xarray}] [--temp-dir TEMP_DIR] --output-file OUTPUT_FILE [--output-simulated-beta] [--min-maf MIN_MAF] [--min-mac MIN_MAC] --h2 H2 [--mix-prop MIX_PROP]
                        [--prop-causal PROP_CAUSAL] [--var-mult VAR_MULT] [--phenotype-likelihood {gaussian,binomial}] [--prevalence PREVALENCE] [--seed SEED]
...
options:
  ...
  --backend {magenpy,plink,bed-reader,xarray}
  ...
  --mix-prop MIX_PROP   Mixing proportions for the mixture density (comma separated). For example, for the spike-and-slab mixture density, with the proportion of causal variants set to 0.1, you can specify: "--mix-prop 0.9,0.1 --var-mult 0,1".
