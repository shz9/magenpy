...
              Version: 0.2.0 | Release date: July 2026
             Author: Shadi Zabad, University of Oxford
          ************************************************
           < Compute LD matrix and store in Zarr format >

usage: magenpy_ld [-h] [--estimator {shrinkage,windowed,sample,block}] --bfile BED_FILE [--keep KEEP_FILE] [--extract EXTRACT_FILE] [--backend {magenpy,xarray,bed-reader,plink}] [--temp-dir TEMP_DIR] --output-dir OUTPUT_DIR [--min-maf MIN_MAF] [--min-mac MIN_MAC] [--genome-build GENOME_BUILD] [--metadata METADATA]
                  [--storage-dtype {float64,float32,int16,int8}] [--compute-spectral-properties] [--compressor {zlib,gzip,lz4,zstd}] [--compression-level COMPRESSION_LEVEL] [--ld-window LD_WINDOW] [--ld-window-kb LD_WINDOW_KB] [--ld-window-cm LD_WINDOW_CM] [--ld-blocks LD_BLOCKS] [--genmap-Ne GENMAP_NE] [--genmap-sample-size GENMAP_SS]
...
options:
  ...
  --estimator {shrinkage,windowed,sample,block}
  ...
  --backend {magenpy,xarray,bed-reader,plink}
  ...
  --metadata METADATA   A comma-separated string with metadata keys and values. This is used to store information about the genotype data from which the LD matrix was computed, such as the biobank/samples, cohort characteristics (e.g. ancestry), etc. Keys and values should be separated by =, such that inputs are in the form of:--metadata
